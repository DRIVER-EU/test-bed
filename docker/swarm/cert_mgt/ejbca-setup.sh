...
# This becomes database.name (type of database server) in generated conf/database.properties
# For example, "mysql" for both MySQL and MariaDB (but database_driver is different)
#database_server_name="mysql"
database_server_name="h2"

# Used only for MariaDB setup
#database_host="localhost"
# This is the name of the database hosted by the Mysql/MariaDB server
#database_name="ejbcatest"

database_driver="h2"

database_url="jdbc:h2:~/ejbcadb;DB_CLOSE_DELAY=-1"

database_username="sa"
database_password="sa"

BASE_DN="OU=WP923,O=DRIVER PROJECT"
...
ejbca_deploy_check() {
  ...
  cat wildfly/standalone/log/server.log
...
# Only for default H2 db
  wildfly_exec "data-source add --name=ejbcads --driver-name=\"${database_driver}\" --connection-url=\"${database_url}\" --jndi-name=\"java:/EjbcaDS\" --user-name=\"${database_username}\" --password=\"${database_password}\""
#  wildfly_exec ":reload"
...
#   generated password, e.g. by using:
...
# Default: h2
database.name=${database_server_name}
...
if [ -f /etc/redhat-release ]; then
  ...
  PKG_INSTALL="apt install unzip openjdk-8-jdk-headless ant ant-optional psmisc bc patch"
  BASE_OS=UBUNTU
  else
